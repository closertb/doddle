...
Operators.push(new Operator('arrayIn', (a, b) => a.includes(b), Array.isArray));
Operators.push(new Operator('arrayNotIn', (a, b) => !Array.isArray(a) || !a.includes(b)));
...
export const systemRuleOfPhone = (rule, value) => {
    const reg = /^[1][3,4,5,6,7,8,9][0-9]{9}$/;
    if (value && !reg.test(value)) {
        return '手机号格式不正确';
    }
};
export const systemRuleOfEmail = (rule, value) => {
    const reg = /^[\w-]+(\.[\w-]+)*@[\w-]+(\.[\w-]+)+$/;
    ;
    if (value && !reg.test(value)) {
        return '邮箱格式不正确';
