...
import { InteractiveCustomMethods, InteractiveVisibleMethods } from './interactive';
...
export interface HFormRuleItem extends Omit<RuleItem, 'validator'> {
    validator?: (rule: HFormInternalRuleItem, value: Value, callback: (error?: string | Error) => void, source: Values, options: ValidateOption) => SyncValidateResult | void;
}
...
export interface ActionsRef {
    deps: string[];
    onChange?: (methods: InteractiveCustomMethods, props?: Record<string, any>) => any;
    rules?: HFormRule;
}
export interface HFormVisibleAction {
    deps: string[];
    onChange?: (methods: InteractiveVisibleMethods, props?: Record<string, any>) => any;
}
export interface HFormCustomAction {
    deps: string[];
    hooks?: FieldsValue['hooks'];
    onChange?: (methods: InteractiveCustomMethods, props?: Record<string, any>) => void;
    rules?: HFormRule;
}
