...
export default class Almanac {
  public allowUndefinedFacts: boolean;
  public factMap: Map<string, any>;
  public factResultsCache: Map<string, any>;
  public pathResolver: (value, path) => any;
  public events: {success: any[]; failure: any[] };
  public ruleResults: any[];
  public callbackMethods: null | TcallbackMethods;
  constructor (factMap, runtimeFacts = {}, options: AnyObject = {}) {
    this.factMap = new Map(factMap)
    this.factResultsCache = new Map() // { cacheKey:  Promise<factValu> }
    this.allowUndefinedFacts = Boolean(options.allowUndefinedFacts)
    this.pathResolver = options.pathResolver || defaultPathResolver
    this.events = { success: [], failure: [] };
    this.callbackMethods = options.callbackMethods || null;
    this.ruleResults = []
  ...
  factValue (factId, params = {}, path = '') {
    // console.log('kkkk', factId, params, path);
