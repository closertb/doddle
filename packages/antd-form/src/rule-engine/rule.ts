...
class Rule extends Emitter {
  ...
  setEvent (event) {
    ...
    this.ruleEvent.result = event.result
    return this
  }
  ...
  evaluate (almanac) {
    ...
    const evaluateCondition = (condition) => {
      if (condition.isBooleanOperator()) {
        ...
        return condition.evaluate(almanac, this.engine.operators, this.engine.getContext())
          .then(evaluationResult => {
            const passes = evaluationResult.result
            condition.factResult = evaluationResult.leftHandSideValue;
            condition.result = passes
            if (condition.isMethod && condition.withResult) {
               ruleResult.setCalResult(condition.factResult);
            }
            return [passes, condition];
          })
      }
    ...
    const evaluateConditions = (conditions, method) => {
      ...
      return Promise.all(conditions.map((condition) => evaluateCondition(condition)))
        .then((conditionResults: [boolean, Condition][]) => {
          // debug('rule::evaluateConditions results', conditionResults)
          return method.call(conditionResults, ([result, condition]) => result === true)
        })
    }
    ...
    const prioritizeAndRun = (conditions, operator) => {
      ...
      for (let i = 0; i < orderedSets.length; i++) {
        ...
        let stop = false
        cursor = cursor.then((setResult: any) => {
    ...
    const processResult = (result) => {
      ...
      // console.log('kkkk result', result, ruleResult);

      const event = result ? 'success' : 'failure'
      return this.emitAsync(event, Object.assign(ruleResult.event, isUndefined(ruleResult.calResult) ? {} : {
        result: ruleResult.calResult
      }), almanac, ruleResult).then(() => ruleResult);
    }
