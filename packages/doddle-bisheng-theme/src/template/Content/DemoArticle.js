...
export default class DemoArticle extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  renderSample() {
    return (
      <div className="page-content frame">
        <div className="frame-header">what</div>
      </div>
    );
  }
  render() {
    ...
    return (
      <div>
        <h1>{title}</h1>
        <div className="page-content markdown">{toReactComponent(content)}</div>
        {demoComponent && <div className="demo-wrapper">{demoComponent}</div>}
        {api && (
        ...
        {!toc || toc.length <= 1 || disableToc ? null : (
          <section className="toc">{toReactComponent(toc)}</section>
        )}
      </div>
    );
  }
